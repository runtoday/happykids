...
const OurStore = () => {
  ...
  const [listProduct, setList] = useState({data:[],loading:true});
  useEffect(()=>{
    
     const fetchData = async () => {
      const response = await fetch('https://localhost:7227/ProductService');
      const data = await response.json();
      setList({data:data,loading:false})
    }    
    if(listProduct.loading === true)
    {
     fetchData()
            ...
            <div className="filter-sort-grid mb-4">
              ...
              <div className="products-;ist pb-5">
                <div className="d-flex gap-10 flex-wrap">
                 {(!listProduct.loading)? listProduct.data.map(product =>
                   <ProductCard key={product.productID} name={product.name}
                  price={product.price} id={product.productID} img ={product.image} grid={grid}/>
                  ) : <p>loading ...</p>}
                </div>
              </div>
            </div>
